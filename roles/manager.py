...
async def add_reviews_handler(call: CallbackQuery, state: FSMContext):
        await call.message.edit_text('Отправьте ссылку на товар или услугу.', reply_markup=None)
...
async def confirmation_handler(call: CallbackQuery, state: FSMContext):
        ...
        if call.data == 'сonfirm':
            profile_link = get_profile_link_from_product_link(data['link'])
